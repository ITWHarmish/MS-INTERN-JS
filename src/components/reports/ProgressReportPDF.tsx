...
const ProgressReportPDF = () => {
  ...
  const externalGuideRemarks = Array.from(
    { length: 4 },
    (_, index) => data?.externalGuideRemarks[index] || ""
  );
  ...
  return loading ? (
    ...
    >
      <div
        style={{ marginBottom: "25px", padding: "120px" }}
        className="report"
      >
        <Button
          id="pdf-button"
        ...
        <div style={{ width: "43rem" }}>
          <div className="section name">
            <div className="name">
              <span>Student Name:</span> {data?.studentName}
            </div>
          </div>
          <div className="section">
            <div className="company">
              <span>Company Name:</span> Toshal Infotech
            </div>
          </div>
          <div className="section">
            <div className="company">
            ...
            <span style={{ marginBottom: "14px" }}>Project Title:</span>{" "}
            {data?.projectTitle}
          </div>
          <div className="section">
            <span>Duration:</span> <span>From:</span>{" "}
            {formatDate(data?.duration?.from)}
            <span style={{ marginLeft: "150px" }}>To:</span>{" "}
            {formatDate(data?.duration?.to)}
          </div>
          ...
          <table className="table">
            ...
            <tbody className="table-tbody">{tasksHtml}</tbody>
          </table>
          ...
          <div className="section single-line">
            ...
            >
              ...
              <span style={{ width: "max-content" }} className="blank-remarks">
                {getRegularityLabel(data?.selfEvaluation?.regularity)}
              </span>
            </div>
            <div
              style={{
              ...
              className="regular"
            >
              <span style={{ marginRight: "3px" }}>Punctuality:</span>{" "}
              {strikeThroughOptions(data?.selfEvaluation?.punctuality)}
            </div>
            <div
              style={{
                display: "flex",
              ...
              className="regular"
            >
              <span style={{ marginRight: "3px" }}>Discipline:</span>{" "}
              {strikeThroughOptions(data?.selfEvaluation?.discipline)}
            </div>
          </div>

          <div className="section">
            <span>Learning Ability:</span>{" "}
            {strikeThroughOptions(data?.selfEvaluation?.learningAbility)}
            <span style={{ marginLeft: "118px" }}>
              Implementation Ability:
            </span>{" "}
            {strikeThroughOptions(data?.selfEvaluation?.implementationAbility)}
          </div>
          ...
          <div>
            ...
            {externalGuideRemarks.map((text, index) => (
              <div key={index} className="line">
                {index + 1}.<div className="blank-remarks">{text}</div>
              </div>
            ))}
          </div>
